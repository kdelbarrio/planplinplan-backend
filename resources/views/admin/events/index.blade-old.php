...
<body>
  ...
  <div class="info">
    ...
    <form id="etl-form" method="POST" action="{{ route('admin.etl.run') }}" class="inline">
      @csrf
      <select name="etl" id="etl-select">
        <option value="kulturklik">Importar Kulturklik</option>
        <option value="experiencias">Importar Experiencias</option>
      </select>
      <button id="etl-run" type="submit" class="btn btn-primary">Ejecutar ETL</button>
    </form>

    <div id="etl-output" style="white-space:pre-wrap; margin-top:12px; color:#222"></div>
  </div>
    ...
    <table>
      ...
      @foreach($events as $e)
        <tr>
          ...
          <td>{{ $e->starts_at?->timezone('Europe/Madrid')->format('Y-m-d H:i') ?? '' }}</td>
